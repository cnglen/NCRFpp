...
def recover_nbest_label(pred_variable, mask_variable, label_alphabet, word_recover):
    ...
    for idx in range(batch_size):
        ...
        for idz in range(nbest):
            each_pred = [label_alphabet.get_instance(pred_tag[idx][idy][idz])
...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    for idx in range(feature_num):
        feature_seq_tensors.append(autograd.Variable(torch.zeros(
    ...
    char_seq_tensor = autograd.Variable(torch.zeros(
        (batch_size, max_seq_len, max_word_len)), volatile=volatile_flag).long()
